...
GIT_HOST ?= github.com/terrafying
...
build:
	...
	@CGO_ENABLED=0 GOOS=linux go build -o build/$(IMAGE_NAME) ./cmd/
...
build-image: build
	...
	@docker build -t $(IMAGE_REPO)/$(IMAGE_NAME):$(IMAGE_TAG) -f build/Dockerfile ./build
